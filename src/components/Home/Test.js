...
import {addReading,bluetoothDiconnect, updateDeviceDetailsByBlutooth, getDevicesForBluetooth, resetDeviceDetails, onDeviceIdChanged, getHistory, getUserDetailsForSettingsForBluetooth} from '../../actions';
...
class BluetoothComponents extends Component {
    constructor() {
        ...
        // alert(JSON.stringify(this.b64DecodeUnicode('Oi0yMTEsIklzQWRhcHRlclBsdWc=')));
    }
...
*/
    error(message) {
        ...
        this.manager.destroy();
        this.manager = new BleManager()
        this.setState({info: "Some thing is wrong"})
        ...
        if (this.props.deviceData.length > 0) {
            ...
            this.manager.startDeviceScan(null,
                null, (error, device) => {
                    ...
                    if (device.name == BLUETOOTH_DEVICE_NAME && device.serviceUUIDs != null && device.serviceUUIDs.length > 0) {
                        console.log(device);
                                ...
                                if ((device_id_iphone == device.id)) {
                                    ...
                                    if (i == this.props.deviceData.length - 1) {
                                        if (device.name === 'SIPL_BLE') {
                                            this.info(`Unable to connect,Please add device( ${device.id} ) first `);
            ...
            this.info("No device Found,Please add device first");
...
*/
    renderHeader() {
        return (
            <HeaderComponent label="Bluetooth" isBackActive={true} isSettingActive={false}/>
        )
    }
...
*/
    ...
    render() {
        if (this.state.isLoading) {
            ...
            if (this.state.isActive) {
                return (
                    <Container style={styles.containerBackgroundColorWithoutHeader}>
                        <Content padder>
                            {this.renderContent()}
...
const mapStateToProps = ({device, setting}) => {
    const {device_id, device_name, tank_name, loading} = device;
    ...
    return {device_id, loading, deviceData, device_name, tank_name, title, from_date, to_date, interval_in_minutes, repeat_duration, error, alert_level_change, repeat,userId
    };
